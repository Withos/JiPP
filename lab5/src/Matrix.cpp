...
void Matrix::set(int n, int m, double val)
{
    try
    {
        if(n<0 || n>m_no_rows || m<0 || m>m_no_columns)
        throw 1;
        my_matrix[n][m]=val;
    }
...
double Matrix::get(int n, int m)
{
    try
    {
        if(n<0 || n>m_no_rows-1 || m<0 || m>m_no_columns-1)
        throw 1;
...
{
    output << m2.m_no_columns << " " << m2.m_no_rows <<"\n"; 
         for (int i = 0; i < m2.m_no_rows; i++) 
        {
            for (int j = 0; j < m2.m_no_columns; j++) 
            {
                output << m2.my_matrix[i][j]<< " ";
            }
            output <<"\n";
        }
}
...
{
    if(m_no_columns!=m2.cols() || m_no_rows!=m2.rows())
    return false;
...
void Matrix::operator+=(Matrix m2)
{
    ...
    {    
        ...
        for (int i=0 ; i<m_no_rows ; i++)
        {
            for (int j = 0 ; j<m_no_columns ; j++)
            {
                this->my_matrix[i][j]=this->my_matrix[i][j]+m2.get(i,j); 
            }
        }
        }
    else
